...
resource "aws_cloudfront_distribution" "nariops" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.nariops.domain_name
        zone_id = aws_cloudfront_distribution.nariops.hosted_zone_id
      }
    },
